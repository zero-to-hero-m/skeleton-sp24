    ...
    public boolean atLeastOneMoveExists() {
        if(emptySpaceExists())return true;
            ...
            for(int i=0;i<size;i++){
                for(int j=0;j<size;j++){
                    int up = i-1>=0?i-1:-1;
                    int right = j+1<size?j+1:-1;
                    if(up !=-1 && board.tile(i,j).value() == board.tile(up,j).value())return true;
                    if(right!=-1 && board.tile(i,j).value() == board.tile(i,right).value())return true;
